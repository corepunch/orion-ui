    ...
    case evPaint:
      ...
      for (uint32_t i = 0; i < cb->cursor_pos; i++) {
        ...
        if (i == win->cursor_pos) {
          ...
          draw_text_small_clipped(texts[i], &item, get_sys_color(brWindowBg), TEXT_PADDING_LEFT);
        } else {
          draw_text_small_clipped(texts[i], &item, get_sys_color(brTextNormal), TEXT_PADDING_LEFT);
        }
      }
      return true;
