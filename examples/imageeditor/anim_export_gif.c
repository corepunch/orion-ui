...
static void write_gif_header(gif_writer_t *gw, int w, int h,
                              const uint32_t *gct, int gct_size) {
  // Find ceil_log2 of gct_size (palette depth − 1)
  int depth = 0;
  while ((1 << depth) < gct_size) depth++;
  if (depth < 1) depth = 1;
  if (depth > 8) depth = 8;
  int gct_entries = 1 << depth;

  gw_bytes(gw, "GIF89a", 6);
  gw_word(gw, (uint16_t)w);
  gw_word(gw, (uint16_t)h);
  // Packed field: global colour table present, colour resolution 7, sorted 0,
  //              global colour table size = depth-1.
  gw_byte(gw, (uint8_t)(0x80 | (7 << 4) | (depth - 1)));
  gw_byte(gw, 0);   // background colour index
  gw_byte(gw, 0);   // pixel aspect ratio

  // Global colour table (RGB triples, not RGBA).
  for (int i = 0; i < gct_entries; i++) {
    uint32_t c = (i < gct_size) ? gct[i] : 0;
...
static void write_netscape_ext(gif_writer_t *gw) {
  ...
  gw_word(gw, 0);    // Loop count 0 = infinite
...
static void write_image_descriptor(gif_writer_t *gw, int w, int h) {
  ...
  gw_word(gw, (uint16_t)w);
  gw_word(gw, (uint16_t)h);
  gw_byte(gw, 0);    // Packed: no local colour table, not interlaced
}
...
bool anim_export_gif(canvas_doc_t *doc, const char *path) {
  ...
  // Build a shared global colour table from the first frame's palette.
  // GIF uses 8-bit indexed colour — we already store frames as indexed.
  const uint32_t *gct = tl->frames[0]->palette;
  int gct_size = 256;

  write_gif_header(&gw, doc->canvas_w, doc->canvas_h, gct, gct_size);
  if (tl->frame_count > 1)
    write_netscape_ext(&gw);
  ...
  for (int fi = 0; fi < tl->frame_count && ok; fi++) {
    ...
    // Ensure we have indexed data to encode.
    const uint8_t *indices = NULL;
    uint8_t *local_indices = NULL;

    if (frame->format == FRAME_FORMAT_INDEXED && frame->data) {
      indices = frame->data;
    } else {
      // Need to expand then re-quantize.
      if (!rgba_tmp) {
      ...
      local_indices = malloc((size_t)doc->canvas_w * (size_t)doc->canvas_h);
      if (!local_indices) { ok = false; break; }
      uint32_t tmp_pal[256];
      /* quantize_rgba_indexed is static in anim.c, accessible in the unity build */
      quantize_rgba_indexed(rgba_tmp, doc->canvas_w, doc->canvas_h,
                             tmp_pal, local_indices);
    ...
    write_image_descriptor(&gw, doc->canvas_w, doc->canvas_h);
